...
import {ISuiteOptions, runTestSuite} from "../suite/suite";

describe("MetaStudioToken", async function () {
  runTestSuite({
    // accounts to test with, accounts[0] being the contract owner
    accounts: await ethers.getSigners(),

    create: async function (options: ISuiteOptions): Promise<Contract> {
      ...
      const logicalContract = await ethers.getContractAt(
        "MetaStudioToken",
    ...
    beforeEach: async function (contract: Contract, options: ISuiteOptions) {
      // const [owner] = options.accounts;
      // // Deployement should trigger ownership changement `__Ownable_init()`
      // // which emit `OwnershipTransferred` event
      // // await expect(proxyContract.deployTransaction)
      // //   .to.emit(proxyContract, "OwnershipTransferred")
      // //   .withArgs(ethers.constants.AddressZero, owner);
    ...
    // token info to test
    name: "MetaStudioToken",
    symbol: "SMV",
    decimals: 18,

    // // initial state to test
    initialSupply: 5_000_000_000,
    // initialBalances: [[accounts[0], 100]],
    // initialAllowances: [[accounts[0], accounts[1], 0]],

    tokensMintedOnCreate: true,
    tokensOwnerAccountIdx: 1,
...
//       const {metaStudioTokenOwner, deployer} = await getNamedAccounts();
//       const Token = await ethers.getContract("MetaStudioToken");
...
//       await deployments.fixture(["MetaStudioToken"]);
//       const {metaStudioTokenOwner} = await getNamedAccounts();
//       const Token = await ethers.getContract("MetaStudioToken");
//       expect(await Token.balanceOf(metaStudioTokenOwner)).to.equal(0);
//     });
//
//     it("Deployment should assign the total supply of tokens to the tokenOwner", async function () {
//       await deployments.fixture(["MetaStudioToken"]);
