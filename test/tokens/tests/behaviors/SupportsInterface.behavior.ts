...
const INTERFACES: {[k: string]: string[]} = {
  ...
  ERC1363Approve: ['approveAndCall(address,uint256)', 'approveAndCall(address,uint256,bytes)'],
  ERC721: [
    'balanceOf(address)',
    'ownerOf(uint256)',
    'approve(address,uint256)',
...
export function shouldSupportInterface(interfaceName: string) {
  describe(`ERC165 is checking ${interfaceName}...`, function () {
    ...
    it("interface's functions are in ABI", function () {
      const fnSignatures = _map(_keys(this.token.interface.functions), function (value: string) {
        return _ERC165([value]);
      });
      for (const fnName of INTERFACES[interfaceName]) {
        const fnSig = FN_SIGNATURES[fnName];
        expect(fnSignatures.filter((sig: string) => sig === fnSig).length).to.equal(1);
      }
    });
  });
}
