...
export function shouldBehaveLikeERC20Transfer(
  ...
  describe('when the recipient is not the zero address', function () {
    describe('when the sender does not have enough balance', function () {
      ...
      it('reverts', async function () {
        await expect(doTransfer(this.token, from, to, amount, this.forwarder)).to.be.revertedWith(
  ...
  describe('when the recipient is the zero address', function () {
    it('reverts', async function () {
      await expect(
        doTransfer(this.token, from, ethers.constants.AddressZero, balance, this.forwarder)
...
export function shouldBehaveLikeERC20TransferFrom(
  ...
  describe('when the token owner is not the zero address', function () {
    describe('when the recipient is not the zero address', function () {
      describe('when the spender has enough allowance', function () {
        beforeEach(async function () {
          await this.token.connect(tokenOwner).approve(getAddress(spender), balance);
        });
        ...
        describe('when the token owner has enough balance', function () {
          ...
          it('transfers the requested amount', async function () {
            await doTransferFrom(this.token, spender, tokenOwner, to, amount, this.forwarder);

            expect(await this.token.balanceOf(getAddress(tokenOwner))).to.be.equal(0);

            expect(await this.token.balanceOf(getAddress(to))).to.be.equal(amount);
          });

          it('decreases the spender allowance', async function () {
            await doTransferFrom(this.token, spender, tokenOwner, to, amount, this.forwarder);

            expect(await this.token.allowance(getAddress(tokenOwner), getAddress(spender))).to.be.equal(0);
          });
          ...
          it('emits an approval event', async function () {
            ...
            await expect(tx)
              ...
              .withArgs(
                ...
                getAddress(spender),
                await this.token.allowance(getAddress(tokenOwner), getAddress(spender))
              );
          });
        });
        ...
        describe('when the token owner does not have enough balance', function () {
          ...
          it('reverts', async function () {
            await expect(
              doTransferFrom(this.token, spender, tokenOwner, to, amount, this.forwarder)
      ...
      describe('when the spender does not have enough allowance', function () {
        ...
        beforeEach(async function () {
          await this.token.connect(tokenOwner).approve(getAddress(spender), allowance);
        });
      ...
      describe('when the spender has unlimited allowance', function () {
        beforeEach(async function () {
          await this.token.connect(tokenOwner).approve(spender.address, ethers.constants.MaxUint256);
        });

        it('does not decrease the spender allowance', async function () {
          await doTransferFrom(this.token, spender, tokenOwner, to, BigNumber.from(1), this.forwarder);
          expect(await this.token.allowance(getAddress(tokenOwner), getAddress(spender))).to.be.equal(
        ...
        it('does not emit an approval event', async function () {
          await expect(
            doTransferFrom(this.token, spender, tokenOwner, to, BigNumber.from(1), this.forwarder)
    ...
    describe('when the recipient is the zero address', function () {
      ...
      it('reverts', async function () {
        await expect(
          doTransferFrom(this.token, spender, tokenOwner, ethers.constants.AddressZero, amount, this.forwarder)
        ).to.be.revertedWith('ERC20: transfer to the zero address');
      });
    });
  });
  ...
  describe('when the token owner is the zero address', function () {
    it('reverts', async function () {
      await expect(
        doTransferFrom(this.token, spender, ethers.constants.AddressZero, to, BigNumber.from(0), this.forwarder)
...
export function shouldBehaveLikeERC20Approve(
  ...
  describe('when the spender is not the zero address', function () {
    describe('when the sender has enough balance', function () {
      ...
      it('emits an approval event', async function () {
        await expect(doApprove(this.token, owner, spender, amount, this.forwarder))
          .to.emit(this.token, 'Approval')
      ...
      describe('when the spender had an approved amount', function () {
        beforeEach(async function () {
          await doApprove(this.token, owner, spender, BigNumber.from(1), this.forwarder);
        });
  ...
  describe('when the spender is the zero address', function () {
    it('reverts', async function () {
      await expect(
        doApprove(this.token, owner, ethers.constants.AddressZero, supply, this.forwarder)
