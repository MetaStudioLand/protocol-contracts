...
import {MAX_UINT256, toBN, ZERO_ADDRESS} from "../../helpers";
...
export async function shouldBehaveLikeERC20Transfer(
  ...
  describe("when the recipient is the zero address", function () {
    it("reverts", async function () {
      // await expectRevert(
      //   doTransfer( from, ZERO_ADDRESS, balance),
      //   `${errorPrefix}: transfer to the zero address`
      // );
      await expect(
        _doTransfer(this.token, from, ZERO_ADDRESS, balance)
      ).to.be.revertedWith(`${errorPrefix}: transfer to the zero address`);
    });
  });
}
...
export async function shouldBehaveLikeERC20Approve(
  ...
  describe("when the spender is not the zero address", function () {
    describe("when the sender has enough balance", function () {
      ...
      describe("when the spender had an approved amount", function () {
        beforeEach(async function () {
          await _doApprove(this.token, owner, spender, toBN(1));
        });
  ...
  describe("when the spender is the zero address", function () {
    it("reverts", async function () {
      // await expectRevert(
      //   _doApprove(this.token, owner, ZERO_ADDRESS, supply),
