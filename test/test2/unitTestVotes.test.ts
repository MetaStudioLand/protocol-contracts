...
import { batchInBlock, Delegation, EIP712Domain, MAX_UINT256, ZERO_ADDRESS } from "../shared/constants";
...
export function unitTestVotes() {
  describe("======== ERC20 VOTES ================================================", async function () {
    ...
    const version = '1';
    const buildData = (chainId: any, verifyingContract: string, message: { delegatee: any; nonce: BigNumber; expiry: BigNumber; }) => ({ data: {
    ...
    it('initial nonce is 0', async function () {
      ...
      describe('set delegation', function () {
        describe('call', function () {
          it('delegation with balance', async function () {
            expect(await this.token.delegates(this.signers.initialHolder.address)).to.be.equal(ZERO_ADDRESS);
    
            const receipt = await this.token.connect(this.signers.initialHolder).delegate(this.signers.initialHolder.address);
            ...
            expect(await this.token.delegates(this.signers.initialHolder.address)).to.be.equal(this.signers.initialHolder.address);
            expect(await this.token.getVotes(this.signers.initialHolder.address)).to.be.equal(this.initialSupply);
            expect(await this.token.getPastVotes(this.signers.initialHolder.address, receipt.blockNumber - 1)).to.be.equal(BigNumber.from(0));
          ...
          expect(await this.token.getPastVotes(this.signers.initialHolder.address, receipt.blockNumber)).to.be.equal(this.initialSupply);
          });
          it('delegation without balance', async function () {
            ...
            expect(
              receipt
               ...
               .not.emit(this.token, 'DelegateVotesChanged')
            expect(await this.token.delegates(this.signers.initialHolder.address)).to.be.equal(this.signers.initialHolder.address);
          });
        });
        describe('with signature', async function () {
          beforeEach(async function () {
            ...
            this.addrWalet = web3.utils.toChecksumAddress( await this.randomWallet.getAddress())            
           this.nonce = 0;
      ...
      describe('change delegation', function () {
        ...
        it('call', async function () {
          expect( await this.token.delegates(this.signers.initialHolder.address)).to.be.equal(this.signers.initialHolder.address);
          const receipt  = await this.token.connect(this.signers.initialHolder).delegate(this.signers.forwarder.address);
          expect(receipt)
          .to.emit(this.token, "DelegateChanged")
          .withArgs(this.signers.initialHolder.address,
            this.signers.initialHolder.address,
           ...
           .withArgs(this.signers.initialHolder.address,
            ...
            BigNumber.from(0));
             ...
             expect(receipt)
            .to.emit(this.token, "DelegateVotesChanged")
            .withArgs(this.signers.forwarder.address,
              BigNumber.from(0),
              this.initialSupply,);
    
          expect(await this.token.delegates(this.signers.initialHolder.address)).to.be.equal(this.signers.forwarder.address);
          ...
          expect(await this.token.getPastVotes(this.signers.initialHolder.address, receipt.blockNumber - 1)).to.be.equal(this.initialSupply);
          expect(await this.token.getPastVotes(this.signers.forwarder.address, receipt.blockNumber - 1)).to.be.equal(BigNumber.from(0));
          const sevenDays = 5 * 24 * 60 * 60; 
          await ethers.provider.send('evm_increaseTime', [sevenDays]);
          await ethers.provider.send('evm_mine',[]);
          
          expect(await this.token.getPastVotes(this.signers.initialHolder.address, receipt.blockNumber)).to.be.equal(BigNumber.from(0));
          expect(await this.token.getPastVotes(this.signers.forwarder.address, receipt.blockNumber)).to.be.equal(this.initialSupply);
        });
      });
      ...
      describe('transfers', function () {
        ...
        it('no delegation', async function () {
          const  receipt  = await this.token.connect(this.signers.initialHolder).transfer(this.signers.recipient.address, 1);
        ...
        it('receiver delegation', async function () {
          ...
          .withArgs(this.signers.initialHolder.address,
           ...
           BigNumber.from(1)
             ...
             expect(receipt)
            .to.emit(this.token, "DelegateVotesChanged")
            .withArgs(this.signers.recipient.address,
              BigNumber.from(0),
              BigNumber.from(1)
              );
          this.holderVotes = BigNumber.from(0);
        ...
        afterEach(async function () {
          ...
          await ethers.provider.send('evm_increaseTime', [sevenDays]);
          await ethers.provider.send('evm_mine',[]);
          expect(await this.token.getPastVotes(this.signers.initialHolder.address, blockNumber)).to.be.equal(this.holderVotes);
          expect(await this.token.getPastVotes(this.signers.recipient.address, blockNumber)).to.be.equal(this.recipientVotes);
        });
      });
      ...
      describe('Compound test suite', function () {
        ...
        describe('numCheckpoints', function () {
          it('returns the number of checkpoints for a delegate', async function () {
            await this.token.connect(this.signers.initialHolder).transfer(this.signers.recipient.address, BigNumber.from(100)); //give an account a few tokens for readability
            ...
            expect(await this.token.checkpoints(this.signers.anotherAccount.address, BigNumber.from(0))).to.be.deep.equal([ t1.blockNumber, BigNumber.from(100) ]);
            expect(await this.token.checkpoints(this.signers.anotherAccount.address, BigNumber.from(1))).to.be.deep.equal([ t2.blockNumber, BigNumber.from(90) ]);
            expect(await this.token.checkpoints(this.signers.anotherAccount.address, BigNumber.from(2))).to.be.deep.equal([ t3.blockNumber, BigNumber.from(80) ]);
            expect(await this.token.checkpoints(this.signers.anotherAccount.address, BigNumber.from(3))).to.be.deep.equal([ t4.blockNumber, BigNumber.from(100) ]);
            ...
            const blockAfter = await ethers.provider.getBlock(blockNumAfter);
            const timestampAfter = blockAfter.timestamp;
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber)).to.be.equal(BigNumber.from(100));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t2.blockNumber)).to.be.equal(BigNumber.from(90));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t3.blockNumber)).to.be.equal(BigNumber.from(80));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t4.blockNumber)).to.be.equal(BigNumber.from(100));
          });
    
          it('does not add more than one checkpoint in a block', async function () {
            await this.token.connect(this.signers.initialHolder).transfer(this.signers.recipient.address, BigNumber.from(100));
            expect(await this.token.numCheckpoints(this.signers.anotherAccount.address)).to.be.equal(BigNumber.from(0));
            ...
            expect(await this.token.numCheckpoints(this.signers.anotherAccount.address)).to.be.equal(BigNumber.from(1));
            expect(await this.token.checkpoints(this.signers.anotherAccount.address, 0)).to.be.deep.equal([ t1.blockNumber, BigNumber.from(80) ]);

            const t4 = await this.token.connect(this.signers.initialHolder).transfer(this.signers.recipient.address, 20);
            expect(await this.token.numCheckpoints(this.signers.anotherAccount.address)).to.be.equal(BigNumber.from(2));
            expect(await this.token.checkpoints(this.signers.anotherAccount.address, 1)).to.be.deep.equal([ t4.blockNumber, BigNumber.from(100) ]);
          });
        });
        ...
        describe('getPastVotes', function () {
          it('reverts if block number >= current block', async function () {
            ...
            expect(this.token.getPastVotes(this.signers.anotherAccount.address, 5e10),)
          ...
          it('returns the latest block if >= last checkpoint block', async function () {
            ...
            await ethers.provider.send('evm_increaseTime', [sevenDays]);
            await ethers.provider.send('evm_mine',[]);
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber)).to.be.equal(this.initialSupply);
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber + 1)).to.be.equal(this.initialSupply);
          });
          ...
          it('returns zero if < first checkpoint block', async function () {
            ...
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber - 1)).to.be.equal(BigNumber.from(0));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber + 1)).to.be.equal(this.initialSupply);
          });
          ...
          it('generally returns the voting balance at the appropriate checkpoint', async function () {
            ...
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber)).to.be.equal(this.initialSupply);
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t1.blockNumber + 1)).to.be.equal(this.initialSupply);
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t2.blockNumber)).to.be.equal(this.initialSupply.sub(10));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t2.blockNumber + 1)).to.be.equal(this.initialSupply.sub(10));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t3.blockNumber)).to.be.equal(this.initialSupply.sub(20));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t3.blockNumber + 1)).to.be.equal(this.initialSupply.sub(20));
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t4.blockNumber)).to.be.equal(this.initialSupply);
            expect(await this.token.getPastVotes(this.signers.anotherAccount.address, t4.blockNumber + 1)).to.be.equal(this.initialSupply);
          });
        });
      });
      ...
      describe('getPastTotalSupply', function () {
        ...
        it('reverts if block number >= current block', async function () {
          ...
          expect(this.token.getPastTotalSupply(5e10))
          .to.emit(this.token, "ERC20Votes: block not yet mined"); 
        });
        ...
        it('returns zero if < first checkpoint block', async function () {
          ...
          expect(await this.token.getPastTotalSupply(blockNumBefore + 1)).to.be.equal(
            this.initialSupply
          );
        });
      });
