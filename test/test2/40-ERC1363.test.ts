...
export function unitTestERC1363(): void {
  describe("======== Contract: ERC1363 ========", async function () {
    ...
    describe('via transferFromAndCall', function () {
      ...
      function transferFromAndCallWithData(token: Contract, from: Address, to: Address, value: BigNumber, opts: SignerWithAddress) {
      ...
      function shouldTransferFromSafely(data: any) {
        describe('to a valid receiver contract', function () {
          ...
          it('should call onTransferReceived', async function () {
            const tx = await this.token.connect(this.signers.initialHolder).increaseAllowance(this.signers.spender.address, initialSupply);
            await tx.wait();
            const receipt = data != null ? await this.token.connect(this.signers.spender)["transferFromAndCall(address,address,uint256,bytes)"](this.signers.initialHolder.address, this.erc1363Receiver.address, initialSupply, DATA) : await this.token.connect(this.signers.spender)["transferFromAndCall(address,address,uint256)"](this.signers.initialHolder.address, this.erc1363Receiver.address, initialSupply);
            expect(receipt)
      ...
      function transferFromWasSuccessful(sender: SignerWithAddress, spender: SignerWithAddress, balance: BigNumber) {
        ...
        describe('when the sender does not have enough balance', function () {
          ...
          describe('without data', function () {
            it('reverts', async function () {
              await expect(
                transferFromAndCallWithoutData(this.token, sender.address, this.erc1363Receiver.address, amount, spender)
              ).to.be.revertedWith(
                'ERC20: insufficient allowance'
              );
            });
        ...
        describe('when the sender has enough balance', function () {
          ...
          describe('with data', function () {
            it('transfers the requested amount', async function () {
              await transferFromAndCallWithData(this.token, sender.address, this.erc1363Receiver.address, amount, spender);
            ...
            it('emits a transfer event', async function () {
              ...
              await tx.wait();
              const logs = await transferFromAndCallWithData(this.token, sender.address, this.erc1363Receiver.address, amount, spender);
              await expect(
                logs
              )
          ...
          describe('without data', function () {
            it('transfers the requested amount', async function () {
              await transferFromAndCallWithoutData(this.token, sender.address, this.erc1363Receiver.address, amount, spender);
            ...
            it('emits a transfer event', async function () {
              const logs = await transferFromAndCallWithoutData(
                this.token, sender.address, this.erc1363Receiver.address, amount, spender
              );
              await expect(
                logs
              )
      ...
      describe('to a receiver that is not a contract', function () {
        it('reverts', async function () {
          const tx = await this.token.connect(signers.initialHolder).increaseAllowance(signers.spender.address, initialSupply);
          await tx.wait();
          await expect(
            transferFromAndCallWithoutData(this.token, signers.initialHolder.address, signers.recipient.address, initialSupply, signers.spender)
          ).to.be.revertedWith('ERC1363Upgradeable: _checkAndCallTransfer reverts');
        });
      });
      ...
      describe('to a receiver contract returning unexpected value', function () {
        ...
        it('reverts', async function () {
          ...
          expect(
            transferFromAndCallWithoutData(this.token, signers.initialHolder.address, this.erc1363Receiver.address, initialSupply, signers.spender)
          ).to.be.revertedWith('ERC1363Upgradeable: _checkAndCallTransfer reverts');
        });
      });
      ...
      describe('to a contract that does not implement the required function', function () {
        it('reverts', async function () {
          await expect(
            transferFromAndCallWithoutData(this.token, signers.owner.address, this.token.address, initialSupply, signers.spender)
          ).to.be.reverted;
        });
      });
    });
    ...
    describe('via transferAndCall', function () {
      ...
      function shouldTransferSafely(data: any) {
        describe('to a valid receiver contract', function () {
          ...
          it('should call onTransferReceived', async function () {
            const receipt = data != null ? await this.token.connect(this.signers.initialHolder)["transferAndCall(address,uint256,bytes)"](this.erc1363Receiver.address, initialSupply, data) : await this.token.connect(this.signers.initialHolder)["transferAndCall(address,uint256)"](this.erc1363Receiver.address, initialSupply);
            expect(
              receipt
            )
              .to.emit(this.erc1363Receiver, "Received")
              .withArgs(signers.initialHolder.address, signers.initialHolder.address, initialSupply, data, 1_000_000_000);
      ...
      function transferWasSuccessful(sender: SignerWithAddress, balance: BigNumber) {
        ...
        describe('when the sender has enough balance', function () {
          ...
          describe('with data', function () {
            ...
            it('emits a transfer event', async function () {
              const logs = await transferAndCallWithData(this.token, receiver, amount, sender);
              await expect(
                logs
              )
                .to.emit(this.token, "Transfer")
          ...
          describe('without data', function () {
            ...
            it('emits a transfer event', async function () {
              const logs = await transferAndCallWithoutData(this.token, receiver, amount, sender);
              await expect(
                logs
              )
                .to.emit(this.token, "Transfer")
      ...
      describe('to a receiver contract returning unexpected value', function () {
        ...
        it('reverts', async function () {
          expect(
            transferAndCallWithoutData(this.token, this.erc1363Receiver.address, initialSupply, this.signers.initialHolder)
          ).to.be.revertedWith(
            'ERC1363Upgradeable: _checkAndCallTransfer reverts'
          );
        });
    ...
    describe('via approveAndCall', function () {
      ...
      function shouldApproveSafely(data: any) {
        describe('to a valid receiver contract', function () {
          ...
          it('should call onApprovalReceived', async function () {
            const receipt = data != null ? await approveAndCallWithData(this.token, this.erc1363Spender.address, initialSupply, signers.initialHolder) : await approveAndCallWithoutData(this.token, this.erc1363Spender.address, initialSupply, signers.initialHolder);
            expect(
              receipt
            )
      ...
      function approveWasSuccessful(sender: SignerWithAddress, amount: BigNumber) {
        ...
        describe('with data', function () {
          ...
          it('emits an approval event', async function () {
            const logs = await approveAndCallWithData(this.token, spender, amount, sender);
            await expect(
              logs
            )
        ...
        describe('without data', function () {
          ...
          it('emits an approval event', async function () {
            const logs = await approveAndCallWithoutData(this.token, spender, amount, sender);
            await expect(
              logs
            )
              .to.emit(this.token, "Approval")
      ...
      describe('to a spender contract returning unexpected value', function () {
        it('reverts', async function () {
          ...
          this.erc1363Spender = contextMockFactoryERC1363Spenderr;
          expect(
            approveAndCallWithoutData(this.token, this.erc1363Spender.address, initialSupply, this.signers.initialHolder)
          ).to.be.revertedWith(
            'ERC1363Upgradeable: _checkAndCallApprove reverts'
          );
        });
      ...
      describe('to a contract that does not implement the required function', function () {
        it('reverts', async function () {
          ...
          await expect(
            approveAndCallWithoutData(this.token, invalidSpender.address, initialSupply, signers.owner)
          ).to.be.reverted;
        });
      });
    });
